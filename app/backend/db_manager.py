...
def get_db_connection(project_id: str, detect_types: bool = False) -> sqlite3.Connection:
...
def init_project_db(project_id: str, project_name: str) -> None:
    ...
    cursor.executemany("INSERT OR REPLACE INTO Project_Info (key, value) VALUES (?, ?)", project_info_initial)
    ...
    default_settings = [
        ...
        ("current_sam_apply_postprocessing", str(config.DEFAULT_APPLY_POSTPROCESSING)), # Store as string
        ("export_format_default", config.DEFAULT_EXPORT_FORMAT),
        ("mask_layers_to_export_default", json.dumps(config.DEFAULT_MASK_LAYERS_TO_EXPORT))
    ]
    for key, value in default_settings:
        if value is not None:
            cursor.execute("INSERT OR IGNORE INTO Project_Settings (setting_key, setting_value) VALUES (?, ?)", (key, value))
...
def update_last_modified(project_id: str) -> None:
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE Project_Info SET value = ? WHERE key = 'last_modified_at_content'",
...
def get_project_setting(project_id: str, setting_key: str) -> Optional[str]:
    conn = get_db_connection(project_id)
    cursor = conn.cursor()
    cursor.execute("SELECT setting_value FROM Project_Settings WHERE setting_key = ?", (setting_key,))
    row = cursor.fetchone()
    conn.close()
...
def set_project_setting(project_id: str, setting_key: str, setting_value: Any) -> None:
    ...
    cursor.execute("INSERT OR REPLACE INTO Project_Settings (setting_key, setting_value) VALUES (?, ?)",
...
# --- Image Sources ---
def add_image_source(project_id: str, source_id: str, type: str, details: Dict, credentials_alias: Optional[str]) -> None:
    conn = get_db_connection(project_id)
    cursor = conn.cursor()
    cursor.execute('''
    INSERT INTO Image_Sources (source_id, type, details, credentials_alias, added_at)
    VALUES (?, ?, ?, ?, ?)
    ''', (source_id, type, json.dumps(details), credentials_alias, datetime.utcnow().isoformat()))
    conn.commit()
    conn.close()
    update_last_modified(project_id)
...
# --- Images ---
def add_image_to_pool(project_id: str, image_hash: str, original_filename: Optional[str],
                        source_id_ref: Optional[str], path_in_source: str,
                        width: int, height: int, status: str = 'unprocessed') -> bool:
    conn = get_db_connection(project_id)
    cursor = conn.cursor()
    try:
        cursor.execute('''
        ...
        ''', (image_hash, original_filename, source_id_ref, path_in_source, width, height, status, datetime.utcnow().isoformat()))
        conn.commit()
...
def update_image_status(project_id: str, image_hash: str, status: str) -> None:
    conn = get_db_connection(project_id)
    cursor = conn.cursor()
    cursor.execute("UPDATE Images SET status = ?, last_processed_at = ? WHERE image_hash = ?",
                   (status, datetime.utcnow().isoformat(), image_hash))
    conn.commit()
    conn.close()
    update_last_modified(project_id)

def get_next_unprocessed_image(project_id: str, current_image_hash: Optional[str] = None) -> Optional[Dict[str, Any]]:
    conn = get_db_connection(project_id)
    cursor = conn.cursor()
    ...
    params = []
    if current_image_hash: # Try to get one *after* the current, if any, to avoid loops. This needs an order.
    ...
    if not row and current_image_hash: # If no unprocessed after current, try any unprocessed
        cursor.execute("SELECT * FROM Images WHERE status = 'unprocessed' ORDER BY added_to_pool_at ASC LIMIT 1")
        row = cursor.fetchone()
...
def save_mask_layer(project_id: str, layer_id: str, image_hash_ref: str, layer_type: str,
                      model_details: Optional[Dict], prompt_details: Optional[Dict],
                      mask_data_rle: Any, metadata: Optional[Dict],
                      is_selected_for_final: bool = False, name: Optional[str] = None) -> None:
    conn = get_db_connection(project_id)
    cursor = conn.cursor()
    cursor.execute('''
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (layer_id, image_hash_ref, layer_type, datetime.utcnow().isoformat(),
          json.dumps(model_details) if model_details else None,
          json.dumps(prompt_details) if prompt_details else None,
          json.dumps(mask_data_rle) if isinstance(mask_data_rle, dict) else mask_data_rle, # RLE can be string or dict for pycocotools
          json.dumps(metadata) if metadata else None,
          is_selected_for_final, name))
    conn.commit()
    conn.close()
    update_last_modified(project_id)

def get_mask_layers_for_image(project_id: str, image_hash: str, layer_type: Optional[str] = None) -> List[Dict[str, Any]]:
    conn = get_db_connection(project_id)
    cursor = conn.cursor()
    ...
    for row in cursor.fetchall():
        layer = dict(row)
        if layer['model_details']: layer['model_details'] = json.loads(layer['model_details'])
        if layer['prompt_details']: layer['prompt_details'] = json.loads(layer['prompt_details'])
        try:
...
if __name__ == '__main__':
    ...
    add_image_to_pool(test_project_id, img_hash, "image1.jpg", source_id, "image1.jpg", 800, 600)
    ...
    print(f"Updated image status: {get_image_by_hash(test_project_id, img_hash)['status']}")

    print(f"Next unprocessed: {get_next_unprocessed_image(test_project_id)}")

    layer_id_1 = "layer_abc"
    save_mask_layer(test_project_id, layer_id_1, img_hash, "automask",
                      {"name": "model_X"}, None, "RLE_DATA_STRING_1", {"score": 0.9})
