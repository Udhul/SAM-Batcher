...
class APIClient {
  ...
  getDownloadProjectDbUrl(projectId) {
    // Returns URL string for direct download via <a> tag
  ...
  async renameProject(projectId, newName) {
    return this._request(`/project/${projectId}`, "PUT", {
  ...
  async loadModel(payload) {
    // payload: { model_size_key?, model_path?, config_path?, apply_postprocessing }
  ...
  async addUploadSource(projectId, filesFormData) {
    return this._request(
      `/project/${projectId}/sources/add_upload`,
      "POST",
      filesFormData,
      null,
      true,
  ...
  async addFolderSource(projectId, folderPath) {
    return this._request(`/project/${projectId}/sources/add_folder`, "POST", {
      path: folderPath,
    });
  }
  async addUrlSource(projectId, url) {
    return this._request(`/project/${projectId}/sources/add_url`, "POST", {
      url: url,
    });
  }
  async addAzureSource(projectId, uri, credentialsAlias = null) {
    ...
    return this._request(
      `/project/${projectId}/sources/add_azure`,
      "POST",
      payload,
    );
  }
  ...
  async setImageExempt(projectId, sourceId, imageHash, exempt = true) {
    return this._request(
      `/project/${projectId}/sources/${sourceId}/exempt_image`,
      "POST",
      { image_hash: imageHash, exempt },
    );
  }
  ...
  async getNextUnprocessedImage(projectId, currentImageHash = null) {
    ...
    return this._request(
      `/project/${projectId}/images/next_unprocessed${query}`,
    );
  }
  async getNextImageByStatus(projectId, statuses, currentImageHash = null) {
    const statusParam = Array.isArray(statuses)
      ? statuses.join(",")
      : String(statuses);
  ...
  async setActiveImage(projectId, imageHash) {
    ...
    return this._request(`/project/${projectId}/images/set_active`, "POST", {
      image_hash: imageHash,
    });
  }
  async updateImageStatus(projectId, imageHash, status) {
    return this._request(
      `/project/${projectId}/images/${imageHash}/status`,
      "PUT",
      { status },
    );
  }
  getImageThumbnailUrl(projectId, imageHash) {
    // Returns URL string
    return `${this.baseUrl}/image/thumbnail/${projectId}/${imageHash}`; // Corrected, assuming endpoint exists
  }
  async getImageData(projectId, imageHash) {
    // If raw image data needed separately
    return this._request(`/project/${projectId}/images/${imageHash}/data`);
  }

  async getImageState(projectId, imageHash) {
    return this._request(`/project/${projectId}/image/${imageHash}/state`);
  }

  async updateImageState(projectId, imageHash, payload) {
    return this._request(
      `/project/${projectId}/image/${imageHash}/state`,
      "PUT",
      payload,
    );
  }
  ...
  async commitMasks(projectId, imageHash, payload) {
    ...
    return this._request(
      `/project/${projectId}/images/${imageHash}/commit_masks`,
      "POST",
      payload,
    );
  }
  ...
  async deleteMaskLayer(projectId, imageHash, layerId) {
    return this._request(
      `/project/${projectId}/images/${imageHash}/layers/${layerId}`,
      "DELETE",
    );
  }
  ...
  async requestExport(projectId, payload) {
    ...
    this._dispatchEvent("api-request-sent", {
      endpoint: `/project/${projectId}/export`,
      method: "POST",
    });
    try {
      ...
      this._dispatchEvent("api-response-received", {
        endpoint: `/project/${projectId}/export`,
        data: { success: true, message: `Export '${filename}' initiated.` },
      });
